...
macro_rules! execute_program {
    ($self: expr, $inputs:expr, $program_string:expr, $function_id_string:expr, $private_key:expr, $cache:expr) => {{
        ...
        web_sys::console::time_stamp_with_data(&"parsing inputs".into());
        ...
        if program_id != "credits.aleo" {
            web_sys::console::time_stamp_with_data(&"Adding program to the process".into());
        ...
        if let Some(proving_key) = $self.proving_key_cache.get(&cache_id) {
            web_sys::console::time_stamp_with_data(&"Loading key from webassembly cache".into());
            process
        ...
        web_sys::console::time_stamp_with_data(&"Creating authorization".into());
        ...
        web_sys::console::time_stamp_with_data(&"Creating execute".into());
        let result = process
            .execute::<CurrentAleo, _>(authorization, &mut StdRng::from_entropy())
            .map_err(|err| err.to_string())?;

        web_sys::console::time_stamp_with_data(&"Creating cache".into());
        if $cache {
...
macro_rules! inclusion_proof {
    ($inclusion:expr, $execution:expr, $url:expr) => {{
        web_sys::console::time_stamp_with_data(&"Preparing execution inclusion proof".into());
        let (assignments, global_state_root) = $inclusion
            .prepare_execution_async::<CurrentBlockMemory, _>(&$execution, &$url)
            .await
            .map_err(|err| err.to_string())?;

        web_sys::console::time_stamp_with_data(&"Proving execution inclusion proof".into());
...
macro_rules! fee_inclusion_proof {
    ($process:expr, $private_key:expr, $fee_record:expr, $fee_microcredits:expr, $submission_url:expr) => {{
        web_sys::console::time_stamp_with_data(&"Preparing fee inclusion proof".into());
        ...
        web_sys::console::time_stamp_with_data(&"Proving fee inclusion proof".into());
        let fee = inclusion
